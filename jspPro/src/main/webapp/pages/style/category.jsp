					...
					<div class="area_tabsort">
						...
						<div class="list_stylehash">
							...
							<c:if test="${ not empty catgList }">
								<c:forEach items="${ catgList }" var="CatgDTO"><a class='<c:if test="${ param.ct_main eq CatgDTO.ct_main }">link_stylehash on</c:if><c:if test="${ param.ct_main ne CatgDTO.ct_main }">link_stylehash</c:if>' href="<%=contextPath %>/pages/style/category.do?ct_main=${ CatgDTO.ct_main }&ec_sub=전체" style="">${ CatgDTO.ct_main }<span class="screen_out">선택됨</span></a></c:forEach>
							</c:if>	
						</div>
						...
						<div class="area_styletag">
							...
							<ul class="list_styletag" style="transform: translate(0px, 0px);">
								<li>
									<a aria-current="page" class="<c:if test='${ param.ec_sub eq "전체" }'>link_styletag on</c:if><c:if test='${ param.ec_sub ne "전체" }'>link_styletag</c:if>" href="<%=contextPath %>/pages/style/category.do?ct_main=${ param.ct_main }&ec_sub=전체" style="">
								...
								<c:if test="${ not empty ecatgCatgList }">
									<c:forEach items="${ ecatgCatgList }" var="EcatgCatgDTO">
										<c:if test='${ param.ct_main eq EcatgCatgDTO.ct_main }'>
											<li>
												<a aria-current="page" class='<c:if test="${ param.ec_sub eq EcatgCatgDTO.ec_sub }">link_styletag on</c:if><c:if test="${ param.ec_sub ne EcatgCatgDTO.ec_sub }">link_styletag</c:if>' href="<%=contextPath %>/pages/style/category.do?ct_main=${ EcatgCatgDTO.ct_main }&ec_sub=${ EcatgCatgDTO.ec_sub }" style="">
					...
					<c:if test="${ not empty elistEcatgList }">
						<c:forEach items="${ elistEcatgList }" var="ElistEcatgDTO">
							<c:if test='${ ( param.ct_main eq ElistEcatgDTO.ct_main ) and param.ec_sub eq ( param.ec_sub eq "전체" ? param.ec_sub : ElistEcatgDTO.ec_sub ) }'>
								<li class="">
									<div class="link_new">
										<a class="link_product" href="<%=contextPath%>/pages/view/view.do?el_num=${ ElistEcatgDTO.el_num }">
										...
										<div class="area_tit">
											<a class="link_tit" href="<%=contextPath%>/pages/view/view.do?el_num=${ ElistEcatgDTO.el_num }">
